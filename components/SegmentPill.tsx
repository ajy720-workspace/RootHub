...
export function SegmentPill({ segment, selected = false, index = 0, onSelect }: SegmentPillProps) {
  return (
    <button
      ...
      className={`animate-slide-up min-h-20 min-w-0 flex-1 basis-[calc(50%-0.375rem)] rounded-lg border px-4 py-3 text-left transition active:scale-[0.99] sm:flex-none sm:basis-auto ${styleByType[segment.type]} ${
    ...
    >
      ...
      <span className="break-words text-lg">{segment.text}</span>
    </button>
  );
}
